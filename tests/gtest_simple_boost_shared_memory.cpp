...
std::map<std::string, boost::interprocess::shared_memory_object>
    shared_memory_map_mock;
...
template <>
bool SimpleBoostSharedMemory::open_or_create<my_class>(
    std::string shared_memory_id)
{

    if (shared_memory_map_mock.find(shared_memory_id) !=
        shared_memory_map_mock.end())
        return false;

    shared_memory_map_mock.emplace(shared_memory_id,
                                   boost::interprocess::shared_memory_object(
                                       boost::interprocess::open_or_create,
                                       shared_memory_id.c_str(),
...
template <>
void SimpleBoostSharedMemory::get_shared_memory<my_class>(
    std::string shared_memory_id,
    my_class &obj)
{
    ...
    boost::interprocess::mapped_region region(sm_element->second,
                                              boost::interprocess::read_write);
    obj = *(static_cast<my_class *>(region.get_address()));
}

template <>
void SimpleBoostSharedMemory::update_shared_memory<my_class>(
    std::string shared_memory_id,
    const my_class &obj)
{
    auto sm_element = shared_memory_map_mock.find(shared_memory_id);
    ...
    boost::interprocess::mapped_region region(
        shared_memory_map_mock[shared_memory_id],
        boost::interprocess::read_write);
...
template <>
bool SimpleBoostSharedMemory::open_or_create<my_complex_class>(
    std::string shared_memory_id)
{
...
template <>
void SimpleBoostSharedMemory::get_shared_memory<my_complex_class>(
    std::string shared_memory_id,
    my_complex_class &obj)
{
    auto sm_element = shared_memory_map_mock.find(shared_memory_id);
    if (sm_element == shared_memory_map_mock.end())
        return;
...
template <>
void SimpleBoostSharedMemory::update_shared_memory<my_complex_class>(
    std::string shared_memory_id,
    const my_complex_class &obj)
{
    auto sm_element = shared_memory_map_mock.find(shared_memory_id);
    if (sm_element == shared_memory_map_mock.end())
        return;
...
template <>
bool SimpleBoostSharedMemory::delete_sm<my_complex_class>(
    std::string shared_memory_id)
{
    if (shared_memory_map_mock.find(shared_memory_id) ==
        shared_memory_map_mock.end())
...
TEST(SharedMemoryTest, test_read_write_and_verify_complex)
{
    ...
    ASSERT_TRUE(
        SimpleBoostSharedMemory::open_or_create<my_complex_class>("page1"));
    ...
    for (int i = 1; i < 10; i++)
    {
        ...
        SimpleBoostSharedMemory::update_shared_memory<my_complex_class>("page1",
                                                                        shm);

        SimpleBoostSharedMemory::get_shared_memory<my_complex_class>("page1",
                                                                     shm_get);
