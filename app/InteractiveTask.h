...
using shared_ptr_thread_safe_sample_queue =
    std::shared_ptr<threadsafe_queue<sample_data>>;
...
class InteractiveTask
{
    ...
    std::function<void(/*binded: const std::string &,*/ std::map<K, V>)>
        set_new_data_func;
...
public:
    InteractiveTask(
        ...
        std::function<std::map<K, V>(/* binded: const std::string & */)>
            _get_new_data_func = nullptr,
        std::function<void(/* binded: const std::string , */ std::map<K, V>)>
            _set_new_data_func = nullptr)
        : task_name(_task_name), inner_queue(std::move(_inner_queue)),
          ...
          periodic_task(
              std::make_shared<PeriodicTask<float>>(task_control_block)),
