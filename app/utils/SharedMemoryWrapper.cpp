...
using InnerAllocator =
    bip::allocator<std::pair<const uint32_t, float>,
...
using SharedString =
    bip::basic_string<char, std::char_traits<char>, CharAllocator>;

using OuterAllocator =
    bip::allocator<std::pair<const SharedString, InnerMap>,
                   bip::managed_shared_memory::segment_manager>;

using SharedMap =
    bip::map<SharedString, InnerMap, std::less<SharedString>, OuterAllocator>;
...
bool SharedMemoryWrapper<T>::init(std::string shm_id)
{
    ...
    {
        std::cout << "Failed to open or create shared memory segment: "
                  << ex.what() << std::endl;
        return false;
    }
    return true;
...
bool SharedMemoryWrapper<T>::update(T db)
{
    MutexType *mutex = segment_.find_or_construct<MutexType>("Mutex")();
    {
        ...
        SharedMap *local_shared_main_db = segment_.find_or_construct<SharedMap>(
            shared_mem_id.c_str())(segment_.get_allocator<OuterAllocator>());
        ...
        for (auto &[comp, inner_map] : db)
        {
            ...
            auto in_ner = InnerMap(inner.begin(),
                                   inner.end(),
                                   segment_.get_allocator<InnerAllocator>());
            SharedString component(comp.c_str(),
                                   local_shared_main_db->get_allocator());
...
T SharedMemoryWrapper<T>::retrieve(std::string _shared_mem_id)
{
    ...
    {
        boost::interprocess::scoped_lock<MutexType> lock(*mutex);
        SharedMap *local_shared_main_db = segment.find_or_construct<SharedMap>(
            shared_mem_id.c_str())(segment.get_allocator<OuterAllocator>());
...
template class SharedMemoryWrapper<
    std::map<std::string, std::map<uint32_t, float>>>;
