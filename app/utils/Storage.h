...
public:
    ...
    void write_entry([[maybe_unused]] std::shared_ptr<HasActMethod> entry,
                     [[maybe_unused]] uint32_t comp)
    {
        //DB[comp].counter++;
        DB[comp] = *entry;
    }

    void read_entry([[maybe_unused]] std::shared_ptr<HasActMethod> entry,
                    [[maybe_unused]] uint32_t comp)
    {
        *entry = DB[comp];
    }

    void process([[maybe_unused]] std::shared_ptr<HasActMethod> entry,
                 [[maybe_unused]] uint32_t comp)
    {
        entry->act();
