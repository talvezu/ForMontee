...
class PriorityQueueElement
    : public std::enable_shared_from_this<PriorityQueueElement>
{
...
public:
    ...
    uint64_t
        invokation_delta; //for the array should be larget than the largest value
    uint32_t idx;
    ...
    PriorityQueueElement(std::initializer_list<int> _targets,
        ...
        : target_motors(_targets), occurrences(_occurrences), values(_values),
          deltas(_deltas), invokation_delta(_invokation_delta)
    {
        idx = 0;
        next_invokation =
            Clock::now() +
            std::chrono::milliseconds(static_cast<int>(invokation_delta));
    }
    ...
    float get_value()
    {
        ...
        if (deltas.size())
        {
            for (unsigned int i = 0;
                 i < static_cast<unsigned int>(values.size());
                 i++)
            {
                values[i] += deltas[i];
            }
...
public:
    bool operator()(const shared_ptr<PriorityQueueElement> &lhs,
                    const shared_ptr<PriorityQueueElement> &rhs)
    {
