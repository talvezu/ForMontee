...
bool open_or_create(std::string shared_memory_id)
{
    ...
    shared_memory_map.emplace(shared_memory_id,
...
void get_shared_memory(std::string shared_memory_id, T &obj)
{
    ...
    boost::interprocess::mapped_region region(
        shared_memory_map[shared_memory_id],
        boost::interprocess::read_write);
...
template bool delete_sm<utils::DB<uint32_t, float>>(
    std::string shared_memory_id);
template void get_shared_memory<utils::DB<uint32_t, float>>(
    std::string shared_memory_id,
    utils::DB<uint32_t, float> &obj);
template void update_shared_memory<utils::DB<uint32_t, float>>(
    std::string shared_memory_id,
    const utils::DB<uint32_t, float> &update);
