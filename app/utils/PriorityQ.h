...
public:
    ...
    void status_report()
    {
        ...
        auto print_to_console = [t, this]() {
            ...
            auto ms =
                std::chrono::time_point_cast<std::chrono::milliseconds>(t);
            ...
            if (this->m_queue.size())
            {
                auto duration_top =
                    (std::chrono::time_point_cast<std::chrono::milliseconds>(
                         this->m_queue.top()->next_invokation))
                        .time_since_epoch();

                // Print the duration in milliseconds
                std::cout << duration.count() << " milliseconds since the epoch"
                          << std::endl;
                std::cout << duration_top.count()
                          << " top milliseconds since the epoch" << std::endl;
            }
        };
    ...
    shared_ptr<T> try_pop()
    {
        ...
        if (!m_queue.empty() && ms > m_queue.top()->next_invokation)
        {
            ...
            auto replace_elem =
                optionalNextInvokationItem<PriorityQueueElement>(elem.get());
            if (replace_elem)
            {
...
private:
    std::priority_queue<shared_ptr<PriorityQueueElement>,
                        std::vector<shared_ptr<PriorityQueueElement>>,
                        my_compare>
        m_queue;
